...
def renamebackup():
    ...
    YESNORENAME = xbmcgui.Dialog().yesno("You have chosen to rename", FOLDER, "", "Confirm action?")	
    if not YESNORENAME:	
        startagain()
    # get new name
    ...
    FOLDERPATH = os.path.join(backupfolder, FOLDER)
    NEWNAMEPATH = os.path.join(backupfolder, NEWNAME)
    count = 0
    while count < 50:
        try:
            os.rename(FOLDERPATH, NEWNAMEPATH)
        except:
            pass
    ...
    if os.path.exists(NEWNAMEPATH):
        printstar()
        print ('%s has renamed %s to %s'% (thisaddon, FOLDER, NEWNAME))
        printstar()
        xbmc.executebuiltin('Notification(%s has been renamed, to %s)'% (FOLDER, NEWNAME) )
    else:		
        printstar()
        print ('Problem with %s.  Could not rename %s'% (thisaddon, FOLDER))
        printstar()
        xbmc.executebuiltin('Notification(Something went wrong, Could not rename %s)'% FOLDER )
    #backtostart
    xbmc.sleep(1000)
    getcurrent()		
...
def deletebackup():
    ...
    YESNODELETE = xbmcgui.Dialog().yesno("You have chosen to delete", FOLDER, "", "Confirm delete?")	
    if not YESNODELETE:	
        startagain()		
    # delete folder
    FOLDERPATH = os.path.join(backupfolder, FOLDER)
    if os.path.exists(FOLDERPATH):
        ...
        while count < 50:
            ...
            if not os.path.exists(FOLDERPATH):
                print ('count (%s) is %d'% (FOLDER, count))
                count = count + 50
            xbmc.sleep(300)
            count = count + 1
    if os.path.exists(FOLDERPATH):
        printstar()
        print ('Problem with %s.  Could not delete %s'% (thisaddon, FOLDER))
        printstar()
        xbmc.executebuiltin('Notification(Something went wrong, Could not delete %s)'% FOLDER )	
    else:
        printstar()
        print ('%s has deleted %s'% (thisaddon, FOLDER))
        printstar()
        xbmc.executebuiltin('Notification(%s, has been deleted)'% FOLDER )
    #backtostart
    xbmc.sleep(1000)
    getcurrent()	
