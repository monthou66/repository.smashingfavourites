...
MULTI = os.path.join(ADDONS, "script.me.pvrmultienable")
EMPTYDBFOLDER = os.path.join(SMASHINGFAVOURITES, "miscfiles", "emptypvrdatabase")
...
#            pvraddon = wantedpvr
            ...
            try:
                ...
                while e < 100:
                    try:
                        ...
                        print ('e is %d'% e)
                        print 'boo hoo'
                        xbmc.executebuiltin('Notification(Attempting to replace, TV database)')
                        xbmc.sleep(300)
                        e = e + 1
            except:
                print ('c is %d'% c)
                print 'boo'
                xbmc.executebuiltin('Notification(Attempting to remove old, TV database)')
                xbmc.sleep(300) 
            c = c + 1
        if c == 1000:
            print 'old tvdb deleted'
        else:
            print 'could not delete old tvdb'
        if e == 1000:
            print 'new tvdb installed'
        else:
            print 'could not install new tvdb'
        exit()
    ...
    elif pvraddon == 'enablemultipvr':
        enablemultipvr()
    elif pvraddon == 'disablemultipvr':
        disablemultipvr()
    else:
        printstar()
...
def enablemultipvr():
    if not xbmc.getCondVisibility('System.HasAddon(script.me.pvrmultienable)'):
        if not os.path.exists(MULTI):
            enablesource = os.path.join(SMASHINGFAVOURITES, "miscfiles", "addons", "myaddons", "markers", "script.me.pvrpermanentenable")
            shutil.copytree(enablesource, MULTI)
        xbmc.executebuiltin( 'UpdateLocalAddons' )
        xbmc.executeJSONRPC('{"jsonrpc":"2.0","method":"Addons.SetAddonEnabled","id":7,"params":{"addonid": "script.me.pvrmultienable","enabled":true}}')		
        printstar()
        print "Multiple pvr use has been enabled"
        printstar()
    else:
        printstar()
        print "Multiple pvr use was already permanently enabled"
        printstar()
    if xbmc.getCondVisibility('System.HasAddon(script.me.pvrmultienable)'):
        xbmc.executebuiltin('Notification(Multiple pvr use, is enabled)')
        exit()
    else:
        xbmc.executebuiltin('Notification(Something, went wrong)')
        printstar()
        print ('%s > enablemultipvr did not work correctly'% thisaddon)
        printstar()
        error()

def disablemultipvr():
    if xbmc.getCondVisibility('System.HasAddon(script.me.pvrmultienable)'):
        xbmc.executeJSONRPC('{"jsonrpc":"2.0","method":"Addons.SetAddonEnabled","id":7,"params":{"addonid": "script.me.pvrmultienable","enabled":false}}')
#        printstar()
#        print "multi pvr is disabled"
#        printstar()
    if not xbmc.getCondVisibility('System.HasAddon(script.me.script.me.pvrmultienable)'):		
        xbmc.executebuiltin('Notification(Only one pvr addon, will be loaded)')
        exit()
    else:
        xbmc.executebuiltin('Notification(Something, went wrong)')
        printstar()
        print ('%s > disablemultipvr did not work correctly'% thisaddon)
        printstar()
        error()	
