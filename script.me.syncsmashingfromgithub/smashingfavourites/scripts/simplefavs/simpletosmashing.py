# -*- coding: utf-8 -*-
# impletosmashing.py - updates the saved folders in smashingfavourites/options
...
tempfolder = os.path.join(SMASHINGTEMP, "buildsmashingfromsimplefavs")
simpledir = os.path.join(ADDONDATA, simplefavourites, 'folders')
smashingdir = os.path.join(SMASHINGFAVOURITES, 'options')
...
def finish():
    ...
    printstar()
    xbmc.executebuiltin('Notification(Smashing Favourites, have been updated)')
    exit()
...
printstar()
# check if SF folder exists in addon_data

if not os.path.exists(simpledir):
    error = ('No folder found at %s'% simpledir)
    errormessage()
