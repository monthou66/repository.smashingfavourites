...
##  timeshift           - jump to
##  getsettings - show which pvr is / are currently enabled. Also shows data enabled if there is more than 1 option.
##  
######################################################################################################################################################
######################################################################################################################################################
...
PERMANENT = os.path.join(MISC, "markers", "pvr.permanentenable.txt")
...
#    global pvraddon, window, channels, guide, disable, refresh, cleardb, reset, permanent, temp, group, data, force, quiet, groupnumber, error, error2
    global pvraddon, window, disable, refresh, cleardb, reset, permanent, group, data, force, quiet, groupnumber, error, error2, arglist
...
def logarguments():
    ...
    print '%d arguments received:'% receivedargs
    number = len(sys.argv)
    arglist = []
    c = 1
    while c < number:
        ...
        print arg
        c = c + 1
    argnumber = len(arglist)
    print ('%d arguments to apply:'% argnumber)
    c = 0
    while c < argnumber:
        arg = arglist[c]
        print arg
...
def getpvrs():
    global disabled, pvrs, pvrnumber, activepvrs, activepvrnumber, activepvr, activepvr2, activepvr3, multiple, error
...
def choosepvr():
    global pvraddon, justvisiting, currentsettings, activepvrdata, activepvrdata2, activepvrdata3, stoppvr, endmessage, error
...
def stoponepvr():
    global quiet, stoppvr
    print 'running stoponepvr()'
    if not home == 'true':
        gohome()
    print ('Stopping %s'% stoppvr)
    if xbmc.getCondVisibility('System.HasAddon(%s)'% stoppvr):
        checkreadytostop()
        xbmc.executeJSONRPC('{"jsonrpc":"2.0","method":"Addons.SetAddonEnabled","id":8,"params":{"addonid":"%s","enabled":false}}'% stoppvr)
        xbmc.sleep(1000)
    if xbmc.getCondVisibility('System.HasAddon(%s)'% stoppvr):
        error = 'Problem in stoponepvr()'
        error2 = ('%s was not disabled.'% stoppvr)
        errormessage() 
    else:
        print ('%s disabled'% stoppvr)
        if not quiet == 'true':
            xbmc.executebuiltin('Notification(%s, has been disabled)'% stoppvr)
