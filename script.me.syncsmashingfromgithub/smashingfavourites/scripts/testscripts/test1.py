# -*- coding: utf-8 -*-
# smashingtosimple.py - updates the saved folders in smashingfavourites/skineditfiles
...
tempfolder = os.path.join(SMASHINGTEMP, "buildsimplefavs")
...
printstar()
# check if SF is installed / enabled
if not xbmc.getCondVisibility('System.HasAddon(%s)' % simplefavourites):
    print ('%s is not installed.'% simplefavourites)
    print ('Attempting to install %s'% simplefavourites)
    xbmc.executebuiltin('InstallAddon(%s)'% simplefavourites)
    xbmc.sleep(1000)
    ...
    while d < 100:
        ...
        if xbmcgui.getCurrentWindowDialogId() == 10101:
            xbmc.sleep(1000)
        elif xbmc.getCondVisibility('System.HasAddon(%s)' % simplefavourites):
            xbmc.executebuiltin('Notification(Installed, %s)'% simplefavourites)
            d = 101
        else:
            d = d + 1
            e = d/5
    ...
    if  not xbmc.getCondVisibility('System.HasAddon(%s)' % simplefavourites):
        error = ('%s was not installed'% simplefavourites)
        errormessage()
    else:
        print ('%s has installed %s'% (thisaddon, simplefavourites))
